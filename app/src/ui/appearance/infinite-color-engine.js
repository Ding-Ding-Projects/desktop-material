/**
 * Desktop Material app — colour engine and translator ported from the documentation hub.
 *
...
;(function (global) {
  ...
  /** Bradford-adapt linear-light XYZ from the sRGB D65 white to CSS D50. */
  function d65ToD50(xyz) {
    return {
      x: 1.0479298208 * xyz.x + 0.0229467933 * xyz.y - 0.0501922295 * xyz.z,
      y: 0.0296278157 * xyz.x + 0.9904344846 * xyz.y - 0.017073825 * xyz.z,
      z: -0.0092430581 * xyz.x + 0.0150551449 * xyz.y + 0.7518742899 * xyz.z,
    }
  }

  /** Bradford-adapt CSS D50 XYZ back to the D65 white used by sRGB. */
  function d50ToD65(xyz) {
    return {
      x: 0.9554734527 * xyz.x - 0.0230985369 * xyz.y + 0.0632593087 * xyz.z,
      y: -0.0283697069 * xyz.x + 1.009995458 * xyz.y + 0.0210413989 * xyz.z,
      z: 0.0123140029 * xyz.x - 0.0205076964 * xyz.y + 1.3303659366 * xyz.z,
    }
  }
  ...
  function splitArguments(body) {
    ...
    if (
      parts.lastIndexOf('/') !== slash ||
      slash === 0 ||
      slash !== parts.length - 2
    ) {
      return { values: [], alpha: undefined, valid: false }
    }
    return {
  ...
  function parse(input) {
    ...
    function exact(count, legacyAlpha) {
      return (
        values.length === count ||
        (legacyAlpha === true &&
  ...
  function alphaSuffix(color) {
    return color.alpha >= 1 ? '' : ' / ' + num(color.alpha, 3)
  }
