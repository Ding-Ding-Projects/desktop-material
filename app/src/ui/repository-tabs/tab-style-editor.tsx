...
> {
  public constructor(props: ITabStyleEditorProps) {
    ...
    this.state = {
      ...
      recentHighlightColors: getStringArray(RecentHighlightColorsKey).filter(
        isValidTabColor
      ),
    }
  }
  ...
  private onClearHighlight = () => {
    this.update({ backgroundColor: undefined })
  }
    ...
    return (
      <div className="tab-style-row tab-style-colors">
        <div className="tab-style-colors-head">
          <span className="tab-style-colors-label">{label}</span>
          <div className="tab-style-color-actions">
            {isHighlight && current !== undefined && (
              <button
                type="button"
                className="tab-style-clear-color"
                onClick={this.onClearHighlight}
              >
                No highlight
              </button>
            )}
