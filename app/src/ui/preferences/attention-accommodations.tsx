...
> {
  ...
  private renderMode(mode: typeof Modes[number]): JSX.Element {
    const settingId = ModeSettingIds[mode.id]
    const descriptionIds = settingExplanationDescriptionIds(settingId)
    const enabled = this.state.preferences.enabled[mode.id]
    const stored = this.state.provenance === 'stored-choice'
    return (
      <div
        key={mode.id}
      ...
      >
        ...
        <SettingExplanation
          ...
          explanation={localize(mode.description, mode.descriptionCantonese)}
