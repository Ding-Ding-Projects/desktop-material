...
type CopyBands = Readonly<
  Record<'plain' | 'light' | 'playful' | 'maximum', Copy>
>
...
const english: CopyBands = {
  plain: {
    ...
    credentialNext:
      'After the wait is cleared, enter your credential normally.',
    ...
    molesPrompt:
      'Hit enough visible targets, then submit after the round ends.',
  ...
  maximum: {
    ...
    attemptsUnchanged:
      'The attempt counter has not received even one bonus point.',
    ...
    tooEarly:
      'The mole round is not done; the server will not accept a speedrun.',
    ...
    molesPrompt:
      'Hit enough real targets, then let the timer finish its dramatic pause.',
    submit: 'Submit answer',
    cancel: 'Return to credential entry',
    remaining: '{count} wait skips remain in this rolling hour.',
    targetCounted: 'Target already counted',
...
export function unlockLadderText(
  ...
  return lookup(key, mode, levels).replace(
    /\{(\w+)\}/g,
