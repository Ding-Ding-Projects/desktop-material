...
export interface IUnlockLadderProps {
  ...
  ) => Promise<IUnlockLadderServiceResult | null> | IUnlockLadderServiceResult | null
 ...
 * The current application has no owning credential lockout surface, so this
 * component is intentionally not mounted until that real parent exists.
 */
...
> {
  ...
  public componentDidUpdate(previousProps: IUnlockLadderProps) {
    if (previousProps.challenge?.challengeId !== this.props.challenge?.challengeId) {
      this.setState({ selectedChoice: null, sums: {}, hits: {}, message: null })
    }
  }
  ...
  ) => unlockLadderText(key, this.props.languageMode, this.props.funnyLevels, variables)

  private submit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const challenge = this.props.challenge
    if (challenge === null || this.state.submitting || this.props.disabled === true) return
  ...
  private answerFor(challenge: IUnlockLadderChallenge): UnlockLadderAnswer | null {
    switch (challenge.payload.kind) {
      ...
      case 'whack-a-mole':
        return {
          kind: 'whack-a-mole',
          hits: Object.entries(this.state.hits).map(([moleId, at]) => ({ moleId, at })),
        }
    }
  }
        ...
        return (
          <fieldset className="unlock-ladder-choice-grid">
            ...
            <RadioGroup<string>
              ...
              radioButtonKeys={challenge.payload.choices.map(choice => choice.id)}
              onSelectionChanged={selectedChoice => this.setState({ selectedChoice })}
              renderRadioButtonLabelContents={choiceId => {
                const choice = challenge.payload.choices.find(item => item.id === choiceId)
          ...
          <fieldset className="unlock-ladder-sums">
            ...
            {challenge.payload.questions.map(question => (
              <label key={question.id} className="unlock-ladder-sum">
                <span>{question.left} {question.operator} {question.right} =</span>
                <TextBox
        ...
        return (
          <fieldset className="unlock-ladder-moles">
            ...
            <p role="timer" aria-live="polite">
              {Math.max(0, Math.ceil((challenge.payload.endsAt - this.state.now) / 1000))}s
            </p>
            <div className="unlock-ladder-mole-grid">
              {challenge.payload.moles.map(mole => {
                const visible =
                  this.state.now >= mole.visibleAt && this.state.now <= mole.visibleUntil
                const hit = this.state.hits[mole.id] !== undefined
                return (
                  <div
                    key={mole.id}
                    className="unlock-ladder-mole-slot"
                    style={{ gridRow: mole.row + 1, gridColumn: mole.column + 1 }}
                  >
                    <Md3TonalButton
                      type="button"
                      className={hit ? 'unlock-ladder-mole hit' : 'unlock-ladder-mole'}
                      label={hit ? '✓' : visible ? '●' : '·'}
                      disabled={!visible || hit}
                      onClick={() => {
                        const record = this.props.onMoleHit
                        if (record === undefined) {
                          this.setState({ hits: { ...this.state.hits, [mole.id]: Date.now() } })
                          return
                        }
    ...
    return (
      ...
      >
        ...
        <p className="unlock-ladder-remaining" aria-live="polite">
          {this.text('remaining', { count: String(Math.max(0, this.props.remainingSkips)) })}
        </p>
