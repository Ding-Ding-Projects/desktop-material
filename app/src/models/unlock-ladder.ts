...
export function getInitialUnlockLadderRung(
  schoolMode: boolean
): UnlockLadderRung {
...
export function canUseUnlockLadder(
  ...
  return (
    timestamps.filter(
      timestamp => now - timestamp < UnlockLadderRollingWindowMs
...
function gradeSums(
  ...
  if (
    answer.kind !== 'arithmetic-sums' ||
    answer.answers.length !== challenge.questions.length
  ) {
    return { outcome: 'incorrect', reason: 'wrong-answer-shape' }
  }
  const byId = new Map(
    answer.answers.map(item => [item.questionId, item.value])
  )
  if (
    byId.size !== answer.answers.length ||
    challenge.questions.some(
      question => byId.get(question.id) !== expectedSum(question)
    )
  ) {
...
function gradeMoles(
  ...
  for (const hit of answer.hits) {
    ...
    if (
      mole === undefined ||
      hit.at < mole.visibleAt ||
      hit.at > mole.visibleUntil
    ) {
  ...
  switch (challenge.payload.kind) {
    case 'dim-sum-choice':
      return answer.kind === 'dim-sum-choice' &&
        challenge.payload.choices.some(
...
export function advanceUnlockLadder(
  ...
  if (state.rung === 'dim-sum') {
    ...
    return {
      ...
      wrongDishCount,
      rung:
        wrongDishCount >= UnlockLadderWrongDishesBeforeSums
          ? 'sums'
          : 'dim-sum',
