...
export class UnlockLadderService {
  ...
  public submit(
    ...
    if (
      stored !== undefined &&
      this.activeChallengeByLockout.get(lockoutId) === submission.challengeId
    ) {
    ...
    const answer =
      stored.challenge.payload.kind === 'whack-a-mole'
        ? {
            ...
            hits: [...stored.recordedHits].map(([moleId, at]) => ({
              moleId,
              at,
            })),
          }
  ...
  public recordMoleHit(request: IUnlockLadderMoleHitRequest): boolean {
    ...
    const now = this.clock()
    if (
      now >= stored.challenge.expiresAt ||
      stored.recordedHits.has(request.moleId)
    ) {
      return false
    }
    const mole = stored.challenge.payload.moles.find(
      item => item.id === request.moleId
    )
  ...
  public pruneExpired(): void {
    ...
    for (const [id, stored] of this.challenges) {
      if (stored.challenge.expiresAt <= now) {
        ...
        if (
          this.activeChallengeByLockout.get(stored.challenge.lockoutId) === id
        ) {
  ...
  private createPayload(
    ...
    if (rung === 'sums') {
      const questions: IUnlockLadderSumQuestion[] = []
    ...
    const moles: ReadonlyArray<IUnlockLadderMole> = Array.from(
      ...
      (_, index) => {
        const visibleAt =
          startedAt + randomInt(0, UnlockLadderMoleRoundDurationMs - 1000)
        return {
          id: `mole-${index + 1}-${this.uuid()}`,
          row: Math.floor(index / 4),
          column: index % 4,
          visibleAt,
          visibleUntil: Math.min(
            endsAt,
            visibleAt + UnlockLadderMoleVisibleWindowMs
          ),
        }
      }
    )
  ...
  private answerFor(payload: UnlockLadderChallengePayload): UnlockLadderAnswer {
    switch (payload.kind) {
      case 'dim-sum-choice':
        ...
        return {
          kind: 'dim-sum-choice',
          choiceId: payload.choices[randomInt(payload.choices.length)].id,
        }
