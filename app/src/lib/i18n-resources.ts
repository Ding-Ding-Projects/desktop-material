...
> = {
  ...
  'classicExperience.explanation.plain':
    '開咗之後，個 app 會用返 Material Design 3 重寫之前嗰個介面：repository 分頁條、經典工具列、側欄同 repository 工作區。閂咗就會用 MD3 新殼，有導覽抽屜同八個目的地。兩邊功能一樣齊，淨係擺位唔同，而且一撳即刻生效。',
  'classicExperience.explanation.light':
    '開咗就攞返 MD3 重寫之前嗰個介面——分頁條、經典工具列、側欄、repository 工作區。閂咗就繼續用新殼、有抽屜同八個目的地。兩邊功能一樣，淨係傢俬擺法唔同，而且一撳即刻見到。',
  'classicExperience.explanation.playful':
    '開咗，舊介面就原原本本行返入嚟：分頁條、經典工具列、側欄、repository 工作區，件件都喺你十年肌肉記憶擺低嗰個位。閂咗，MD3 新殼就再接手，帶埋佢個抽屜同八個目的地。兩邊冇邊個做得到啲嘢另一邊做唔到；撳落去即刻生效，唔使重開，唔使搞儀式。',
  'classicExperience.explanation.maximum':
    '開咗，成個重寫之前嘅介面就施施然行返去佢個舊位——分頁條、經典工具列、側欄、repository 工作區，成班人齊齊整整，一個唔少，全部喺你走嗰陣擺低嗰度。閂咗，MD3 新殼就收返個場，帶住佢個導覽抽屜同八個目的地。冇任何一樣功能係淨係得一邊有：經典外殼有嘅嘢全部搬咗入新殼，新殼加嘅嘢喺經典佈局度一樣開得。撳落去嗰秒即刻生效——唔使重開、唔使重新啟動、更加唔使人叫你「試吓熄咗再開」。',
  'classicExperience.boundaryNote':
    '兩個佈局冇邊個去得到啲地方另一邊去唔到。開咗經典介面嗰陣，另外嗰個「顯示舊版工具列」設定就唔再適用：工具列本身就係呢個佈局嘅一部分，唔係 MD3 面板上面嗰條額外橫額。',
  'classicExperience.provenanceDefault':
    '呢部電腦未記低過任何選擇，所以用緊出廠設定：{value}。',
