...
export async function withRepositoryIndicatorProgressCleanup<T>(
  operation: () => Promise<T>,
  clear: () => void
): Promise<T> {
  try {
  ...
  private scheduleRefresh() {
    if (this.running && this.refreshTimeoutId === null) {
      ...
      this.refreshTimeoutId = window.setTimeout(
        () =>
          void this.refreshAllRepositories().catch(error =>
            log.error(
              '[RepositoryIndicatorUpdater] Refresh cycle failed unexpectedly',
              error
            )
          ),
        timeout
    ...
    try {
      while (this.running && (repository = getNextRepository()) !== undefined) {
        try {
          ...
          log.error(
            `[RepositoryIndicatorUpdater] Could not refresh repository ${repository.id}; continuing`,
            error
          )
        }
