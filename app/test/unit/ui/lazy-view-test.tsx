...
describe('LazyView copy', () => {
  it('names the surface in every funny-level band of both languages', () => {
    ...
    for (const band of bands) {
      for (const catalog of [englishTranslations, cantoneseTranslations]) {
        assert.match(catalog[`lazyView.loading.${band}`], /\{name\}/)
        assert.match(catalog[`lazyView.failedBody.${band}`], /\{name\}/)
      }
    }
  })
  ...
  it('keeps the failure facts out of the funny-level bands', () => {
    ...
    for (const catalog of [englishTranslations, cantoneseTranslations]) {
      assert.match(catalog['lazyView.failedTitle'], /\{name\}/)
      assert.match(catalog['lazyView.failedDetail'], /\{error\}/)
      assert.ok(catalog['lazyView.retry'].length > 0)
      assert.match(catalog['lazyView.notificationBody'], /\{name\}/)
      assert.match(catalog['lazyView.notificationBody'], /\{error\}/)
    }
  })
})
