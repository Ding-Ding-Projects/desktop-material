...
- **Release gates** — the manual Super Express emergency lane runs no tests and
  ...
  Windows lane uses `windows-2022`, and the TUI lane uses `ubuntu-latest`,
  keeping the workflow schedulable before any job starts. Ordinary CI and
  tested Express remain the default gates; release pull requests target the
  Windows product's `main` default branch.
