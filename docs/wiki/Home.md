...
| [Guided Feature Gallery](Feature-Gallery) | Machine-checked target of one distinct screenshot for each of 84 Windows visual scenes; the current-source updater frame is accepted and published as its own slot, while five retained Linux/Xvfb assets stay explicitly archived outside the set. |
