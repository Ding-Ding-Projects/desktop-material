...
- **Release gates** — the manual Super Express emergency lane runs no tests and
  ...
  optional release. Preparation and publication use `ubuntu-latest`, the
  Windows lane uses `windows-2022`, and the Linux TUI lane uses
  `ubuntu-latest`, keeping the workflow schedulable before any job starts.
