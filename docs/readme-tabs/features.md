...
- Dispatch the separate **Super Express Release** workflow only for an emergency direct lane: it runs no unit, script, TUI, lint, type, parity, smoke, or E2E tests, then builds Windows x64 and Linux TUI packages in parallel through their own reusable workflows. Each packaging workflow also has a direct `workflow_dispatch` action for a packaging-only recovery artifact; only the combined dispatcher publishes. It restores the exact desktop dependency cache, uses a native Ubuntu TUI runner, writes notes from the checked-out commit, verifies every installer/feed/package asset, retains the complete payload, and publishes one uniquely versioned immutable combined Release for the exact dispatched `main` commit. Keeping one publisher preserves the shared `latest` update/bootstrap feed. Ordinary CI and tested Express remain the default gates. Automatic and Super Express packages share one Squirrel-monotonic `z` version namespace, and only the greatest release for revalidated current `main` can own the update feed
