...
- Dispatch the separate **Super Express Release** workflow only for an emergency direct lane: it runs no unit, script, TUI, lint, type, parity, smoke, or E2E tests, then builds Windows x64 and Linux TUI packages in parallel through their own reusable workflows. The direct Windows `workflow_dispatch` action preserves its verified artifact, keeps packaging on the trusted self-hosted Windows runner, and publishes a uniquely versioned immutable non-latest Windows Release from `ubuntu-latest`; the direct Linux TUI action remains packaging-only, and reusable calls remain artifact-only. The combined dispatcher restores the exact desktop dependency cache, runs every job on the registered self-hosted Windows or Linux runner, writes notes from the checked-out commit, verifies every installer/feed/package asset, retains the complete payload, and publishes one uniquely versioned immutable combined Release for the exact dispatched `main` commit. If a local packaging runner is unavailable, the affected release queues or fails; the direct Windows publisher does not depend on the local WSL runner. Keeping the combined publisher as the only cross-platform publisher preserves the shared `latest` update/bootstrap feed. Ordinary CI and tested Express remain the default gates. Automatic and Super Express packages share one Squirrel-monotonic `z` version namespace, with a run-attempt suffix for reruns, and only the greatest release for revalidated current `main` can own the update feed
