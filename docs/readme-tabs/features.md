...
- Browse GitHub Actions runs in the repository rail, filter by workflow/branch/event/status, re-run all or failed jobs, inspect jobs and steps, securely download and search logs, search each loaded artifact catalog with fuzzy/substring/safe-regex modes, and dispatch workflows with inputs. The manual **CI Windows** dispatch offers a cloud/self-hosted choice for the desktop build and packaged smoke jobs, with the fixed trusted `self-hosted`/`Windows`/`X64`/`desktop-material-windows-local` label set and a cloud default. Actions caches remain listable, searchable, and deletable; cache-archive download is labeled unavailable because GitHub exposes no supported download API, while workflow artifacts retain their verified download path
