...
- Dispatch the separate **Super Express Release** workflow only for an emergency direct lane: it runs no unit, script, TUI, lint, type, parity, smoke, or E2E tests and goes straight to building and packaging Windows x64. It restores the exact dependency cache, writes notes from the checked-out commit, verifies every installer/feed asset, retains the complete payload, and publishes a uniquely versioned immutable Release for the exact dispatched `main` commit. Ordinary CI and tested Express remain the default gates. Automatic and Super Express packages share one Squirrel-monotonic `z` version namespace, and only the greatest release for revalidated current `main` can own the update feed
