...
- Set up a repository-scoped Windows runner with a searchable rich GitHub account picker. Public repositories proceed only after the immutable workflow audit proves that untrusted events cannot reach the managed labels; unknown visibility and unsafe workflow targeting remain blocked. Release notes are shown in their own collapsible section rather than inside Release details.
