...
- **Active — fixed verifier runtime and IPC:** add the closed-stdin, cancellable subprocess boundary, selected-account bundle orchestration, immediate subject re-extraction/digest recheck, zero-bundle handling after rehash, output/deadline ceilings, exact process-tree and Temp cleanup, and normalized IPC outcomes. No executable, arguments, token, endpoint, raw archive path, or raw output crosses into the UI. Local `--bundle` mode avoids attestation API fetching but can still initialize trust through configured Sigstore TUF mirrors, so it is not described as network-disconnected verification.
...
- **Active — remaining verifier and UI adversarial tests:** the safe-subject and projected-result slices now cover stored/deflate members, descriptors, traversal, malformed UTF-8, links, compression ratio, header/CRC/changed-byte failures, cross-sender denial, cancellation, release invalidation, exact trust mismatches, malformed/oversized projected JSON, multi-attestation evidence, tenant issuer derivation, and Temp cleanup. The subprocess/runtime and UI slices still require account-change, unsupported-environment, stale-response, process cancellation/cleanup, focus, responsive-layout, and normalized-outcome coverage.
