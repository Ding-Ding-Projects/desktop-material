...
| **Active** | Expand named GitHub functions on a hardened transport | Native pull-request compose/review/create, Actions artifact download with local digest and attestation-presence context, and effective branch-rules inspection passed their production UI gate at `9e946fd527`. Server-filtered run pagination and compact artifact pagination are now implemented as explicit named load-more workflows with exact-account routing, cancellation, de-duplication, retained-page retry, and contained responsive pagers; the 74/74 focused artifact/run API, parser, store, React, account-routing, and style checks pass. Job pagination, cryptographic attestation verification, a Pull Request Center, an Issue Hub, and a Release Manager follow |
...
| **P0 — Actions pagination** | Load later workflow-run and artifact pages through named app controls; apply run filters at the provider; preserve older run pages across polling; retain loaded artifact cards when a later page fails; cancel stale requests; de-duplicate shifted pages; contain both pagers at narrow widths | **Implemented; 74/74 focused checks pass. Exact production UI interaction and screenshot verification is the remaining gate before this row moves to Done** |
| **P0 — active product wave** | Job pagination; cryptographic attestation verification; Pull Request Center; Issue Hub; Release Manager; Actions approvals and caches | **In progress** |
