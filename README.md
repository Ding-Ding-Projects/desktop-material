...
| **Active — off-screen verification** | Build and interactively verify every changed UI off-screen | The exact MCP build command passes for the current branch, and regular-width interaction has exercised bundled Repository Tools, create-only bundle import, and the corrected GitHub Notifications signed-out state. Compact-width checks and final evidence promotion are still underway |
