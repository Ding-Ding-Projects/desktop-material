...
- **Active — selected-account store orchestration:** connect these authoritative reads and the bounded bundle request to the repository-selected account, invalidate operations on account/repository/download/policy changes, and add the main-process-only selected-credential broker. GitHub.com can verify a local bundle without an account token once trust material is available. GHE.com verification currently returns **Unavailable** because GitHub CLI 2.96 still resolves the trust domain in bundle mode; it remains unavailable until the selected GHE.com credential is injected main-process-only. Tokens remain excluded from IPC, React state, logs, and Temp files.
