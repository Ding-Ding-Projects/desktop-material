...
Verification of source commit `84257ff5a5e05d48cc2440ef1b84446c23933930`:

- 50 focused tests passed across repository metadata, clone sort order, and
  clone staging/recovery (29 sorting checks and 21 recovery checks).
- Full TypeScript `--noEmit` check passed.
- Local Windows production-configuration bundle and runnable assembly passed.
  The interaction build disabled minification and used transpile-only loaders
  after the separate full TypeScript check. Release configuration is unchanged.
- The existing clone-dialog scene exercised all seven choices, verified each
  saved selection, restored A-to-Z, and checked the control's containment in
  the real built application at 1440 by 960, scale 1, English, light and dark.
  Both runs reported zero provider mutations. The fixture contains one
  repository; multi-row ordering is covered by the focused tests.
- Capture-driver contract: 60 tests passed after moving path masking after
  sort interactions so a rerender cannot restore a machine-specific path.

The stale direct-clone journal repair preserves verified recovery paths and
permits clearing only journal metadata when filesystem inspection proves that
no staging or promotion data exists. Occupied destination contents remain
unchanged. Both fixes are integrated into `main`, and the remote SHA was
verified. Issue #240 and Discussion #241 track delivery.

Raw light/dark captures were inspected and retained in the task-owned local
run. They are not promoted into the public gallery: a complete version-1
build/capture receipt was not generated by this legacy driver. They do not
claim installer execution, a complete accessibility matrix, or a release
verdict. Existing lower path-field/footer overlap remains outside the new
sort control's containment proof. Windows workflow 35558908093 was still
running at this update. Temporary checkouts remain retained while delivery
and evidence closeout are incomplete.
