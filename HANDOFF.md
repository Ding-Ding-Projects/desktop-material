...
This Windows Electron fix lives in the new linked worktree branch
`codex/job-log-404-fix` until integration. GitHub can briefly return `HTTP 404`
...
Remote CI, default-branch integration, and release publication are not claimed
until the commit lands and the remote reports its result.
