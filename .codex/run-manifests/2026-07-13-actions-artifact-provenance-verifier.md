...
The authoritative metadata and exact run-attempt checkpoint is complete from clean source `7022dbffa492b1f5881cba5f2ea592be847faa75`. Its bounded implementation allowlist is the shared provenance policy, one new purpose-specific metadata/ref resolver module, four named API reads plus one same-instance resolver method, the projected-result parser, focused tests, this manifest, and the README roadmap. Store orchestration, a credential broker, React UI, screenshots, wiki, and Pages remain outside this backend-only checkpoint.
...
Sixty-one focused metadata, API, policy, projected-result, runtime, and client tests, the 126-test combined artifact/provider/account regression, full TypeScript, focused Prettier, and scoped ESLint pass for this checkpoint. The fixed runner arguments remain unchanged. Selected-account store orchestration, account-change/stale-result invalidation, the main-process-only selected-credential broker, React/accessibility/style checks, the production build, hidden-desktop interaction, screenshots, wiki, and Pages remain with later checkpoints. GitHub.com local bundle verification may still fetch Sigstore TUF metadata; GHE.com remains **Unavailable** until the selected credential is injected main-process-only.
...
- **Active — selected-account store orchestration:** connect these authoritative reads and the bounded canonical bundle response through the repository-selected API instance; invalidate stale work on repository/account/download/policy changes; and, for GHE.com, add a main-process-only selected-credential broker so GitHub CLI 2.96 can resolve the trust domain. No token may enter IPC, React state, logs, or Temp files.
...
- **Active — remaining orchestration and UI tests:** raw ZIP safety, registry, transfer/client, authoritative metadata/API/ref resolution, trust policy, signer metadata, exact projected-result invocation, opaque verifier IPC/client, fixed subprocess behavior, service cancellation/concurrency, shutdown, and Temp cleanup are complete. Store/account-change and stale-result orchestration plus credential-broker, React, accessibility, style-contract, deterministic-provider, and production gates remain with their implementation slices.
