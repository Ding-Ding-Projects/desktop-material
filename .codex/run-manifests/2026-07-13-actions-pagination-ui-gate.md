...
Main-repository evidence and the two PNGs were committed and pushed at `1d81472595b1e01ff457425668cd8afa41f3bf2f`; local, tracking, and direct remote SHAs matched.

The separate wiki's existing extra guidance was preserved while Home, User Guide, and both local image assets were merged. Wiki `master` was committed and pushed at `2585cf7977b14d5792a1addb8b9a7c9f944e1e84`; local, tracking, and direct remote SHAs matched. Cache-busted live Home and User Guide renders showed the new pagination copy and both image links, and both raw PNG URLs resolved.

Pages workflow-dispatch run `29270933754` checked out exact source `1d81472595b1e01ff457425668cd8afa41f3bf2f`. Its build job configured Pages, assembled the publish directory, and uploaded the `github-pages` artifact successfully. The downloaded 3,051,520-byte tar passed traversal/absolute-path inspection and contained both PNGs with the exact tracked byte sizes and hashes above. The deploy job was correctly rejected because `mega-feature-update` is not allowed by the protected `github-pages` environment. No protection was bypassed and the feature branch was not merged merely to publish.

Cleanup completed through the saved ledger: revalidated HWND `77530592` closed gracefully; exact app PID `920` exited; exact provider PIDs `3272`/`7596` exited; ports `62907` and `54054` had no listeners; the unique development credential was present before deletion, deleted successfully, and read back absent; the hidden desktop was closed exactly once; and the containment-checked owned Temp root was removed and confirmed absent. The visible desktop, normal Desktop Material profile, and public GitHub provider state were never touched.
