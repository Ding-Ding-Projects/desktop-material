...
- **Super Express Release fast lane**: A workflow_dispatch-only emergency
  ...
  separate zero-test Windows x64 and Linux TUI packaging workflows in parallel.
  Preparation and publication use `ubuntu-latest`, the Windows package uses
  `windows-2022`, and the Linux TUI package uses `ubuntu-latest`, keeping
  runner placement schedulable and explicit. Each packaging workflow also
  exposes a direct, artifact-only workflow_dispatch action for recovery builds.
  The lanes restore the desktop dependency cache where
