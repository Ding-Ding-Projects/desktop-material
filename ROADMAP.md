...
| CI update-coming-soon status and exact-SHA Release notes | **Local acceptance complete; remote verification pending** | The updater derives only the configured HTTPS GitHub feed, requires the exact `Windows x64` packaging job—not only prerequisite CI—to be in progress for the same run ID/SHA, and accepts the status only when GitHub also proves that SHA is ahead of the installed exact SHA. It keeps the state transient, localizes it in English/Cantonese/bilingual modes, and fences stale async results behind real updater events. The release workflow generates at most 50 sanitized commit subjects from the previous published tag through `RELEASE_TARGET_SHA`, caps its body, and uses `body_path` for the same exact publication target. CI invocations have unique per-run concurrency groups, and all repository workflows reject cancellation of older in-progress runs; shared installer/Pages groups queue safely. Focused updater, i18n, workflow, and generator checks pass 32/32; app/script TypeScript pass. Integration, remote CI, and a published sample Release remain. |
